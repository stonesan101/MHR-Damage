define([ "exports" ], (function(e) {
    ...
    const n = (e, t, r) => {
        if ("function" !== typeof e[t]) throw r.expectedMethod = t, new s("missing-a-method", r);
    }, o = (e, t) => {
        if (!Array.isArray(e)) throw new s("not-an-array", t);
    }, i = (e, t, r) => {
        if (!(e instanceof t)) throw r.expectedClassName = t.name, new s("incorrect-class", r);
    }, c = (e, t, r) => {
        if (!t.includes(e)) throw r.validValueDescription = `Valid values are ${JSON.stringify(t)}.`, 
        new s("invalid-value", r);
    }, h = (e, t, r) => {
        if (typeof e !== t) throw r.expectedType = t, new s("incorrect-type", r);
    };
    try {
        self["workbox:routing:6.5.3"] && _();
    } catch (e) {}
    const l = [ "DELETE", "GET", "HEAD", "PATCH", "POST", "PUT" ], u = e => e && "object" == typeof e ? (n(e, "handle", {
    ...
    class d {
        constructor(e, t, r = "GET") {
            h(e, "function", {
                ...
                paramName: "method"
            }), this.handler = u(t), this.match = e, this.method = r;
        }
        setCatchHandler(e) {
    ...
    const f = e => new URL(String(e), location.href).href.replace(new RegExp(`^${location.origin}`), "");
    class g {
        constructor() {
        ...
        handleRequest({request: e, event: r}) {
            ...
            const l = e.method;
            if (!c && this._defaultHandlerMap.has(l) && (h.push(`Failed to find a matching route. Falling back to the default handler for ${l}.`), 
            c = this._defaultHandlerMap.get(l)), !c) return void t.debug(`No route found for: ${f(a)}`);
            let u;
            t.groupCollapsed(`Router is responding to: ${f(a)}`), h.forEach((e => {
            ...
            return u instanceof Promise && (this._catchHandler || d) && (u = u.catch((async s => {
                if (d) {
                    t.groupCollapsed(`Error thrown when responding to:  ${f(a)}. Falling back to route's Catch Handler.`), 
                ...
                if (this._catchHandler) return t.groupCollapsed(`Error thrown when responding to:  ${f(a)}. Falling back to global Catch Handler.`), 
        ...
        findMatchingRoute({url: e, sameOrigin: r, request: a, event: s}) {
            ...
            for (const o of n) {
                ...
                if (i) return i instanceof Promise && t.warn(`While routing ${f(e)}, an async matchCallback function was used. Please convert the following route to use a synchronous matchCallback function:`, o), 
        ...
        unregisterRoute(e) {
            if (!this._routes.has(e.method)) throw new s("unregister-route-but-not-found-with-method", {
                method: e.method
            });
    ...
    function y(e, r, a) {
        ...
        if ("string" == typeof e) {
            ...
            {
                if (!e.startsWith("/") && !e.startsWith("http")) throw new s("invalid-string", {
                    moduleName: "workbox-routing",
                    funcName: "registerRoute",
                    paramName: "capture"
                });
                const r = e.startsWith("http") ? o.pathname : e, a = "[*:?+]";
                new RegExp(`${a}`).exec(r) && t.debug(`The '$capture' parameter contains an Express-style wildcard character (${a}). Strings are now always interpreted as exact matches; use a RegExp for partial or wildcard matches.`);
            }
            n = new d((({url: r}) => (r.pathname === o.pathname && r.origin !== o.origin && t.debug(`${e} only partially matches the cross-origin URL ${r.toString()}. This route will only handle cross-origin requests if they match the entire URL.`), 
            r.href === o.href)), r, a);
        } else if (e instanceof RegExp) n = new p(e, r, a); else if ("function" == typeof e) n = new d(e, r, a); else {
            if (!(e instanceof d)) throw new s("unsupported-route-type", {
                moduleName: "workbox-routing",
                funcName: "registerRoute",
                paramName: "capture"
            });
    ...
    }, R = e => [ N.prefix, e, N.suffix ].filter((e => e && e.length > 0)).join("-"), v = e => {
        (e => {
            for (const t of Object.keys(N)) e(t);
        })((t => {
            "string" == typeof e[t] && (N[t] = e[t]);
        }));
    }, $ = e => e || R(N.precache), C = e => e || R(N.runtime);
    function x(e, t) {
        const r = new URL(e);
    ...
    class k {
        constructor(e, t) {
            this._cacheKeys = {}, i(t.event, ExtendableEvent, {
                moduleName: "workbox-strategies",
                className: "StrategyHandler",
                funcName: "constructor",
                paramName: "options.event"
            }), Object.assign(this, t), this.event = t.event, this._strategy = e, this._handlerDeferred = new q, 
        ...
        async fetch(e) {
            ...
            if ("navigate" === a.mode && r instanceof FetchEvent && r.preloadResponse) {
                ...
                if (e) return t.log(`Using a preloaded navigation response for '${f(a.url)}'`), 
                e;
            }
            ...
            try {
                ...
                t.debug(`Network request for '${f(a.url)}' returned a response with status '${e.status}'.`);
                ...
                throw t.log(`Network request for '${f(a.url)}' threw an error.`, e), n && await this.runCallbacks("fetchDidFail", {
                    error: e,
                    event: r,
        ...
        async cachePut(e, r) {
            ...
            {
                if (o.method && "GET" !== o.method) throw new s("attempt-to-cache-non-get-request", {
                    url: f(o.url),
                    method: o.method
                });
                const e = r.headers.get("Vary");
                e && t.debug(`The response for ${f(o.url)} has a 'Vary: ${e}' header. Consider setting the {ignoreVary: true} option on your strategy to ensure cache matching and deletion works as expected.`);
            }
            if (!r) throw t.error(`Cannot cache non-existent response for '${f(o.url)}'.`), 
            new s("cache-put-with-no-response", {
                url: f(o.url)
            });
            const i = await this._ensureResponseSafeToCache(r);
            if (!i) return t.debug(`Response '${f(o.url)}' will not be cached.`, i), !1;
            const {cacheName: c, matchOptions: h} = this._strategy, l = await self.caches.open(c), u = this.hasCallback("cacheDidUpdate"), d = u ? await async function(e, t, r, a) {
                const s = x(t.url, r);
                if (t.url === s) return e.match(t, a);
                const n = Object.assign(Object.assign({}, a), {
                    ignoreSearch: !0
                }), o = await e.keys(t, n);
                for (const t of o) if (s === x(t.url, r)) return e.match(t, a);
            ...
            try {
                ...
                if (e instanceof Error) throw "QuotaExceededError" === e.name && await async function() {
                    t.log(`About to run ${T.size} callbacks to clean up caches.`);
        ...
        async getCacheKey(e, t) {
            ...
            if (!this._cacheKeys[r]) {
                let a = e;
                for (const e of this.iterateCallbacks("cacheKeyWillBeUsed")) a = U(await e({
                    mode: t,
                    request: a,
                    event: this.event,
                    params: this.params
        ...
        * iterateCallbacks(e) {
            for (const t of this._strategy.plugins) if ("function" == typeof t[e]) {
                const r = this._pluginStateMap.get(t), a = a => {
    ...
    class E {
        constructor(e = {}) {
            this.cacheName = C(e.cacheName), this.plugins = e.plugins || [], this.fetchOptions = e.fetchOptions, 
        ...
        handleAll(e) {
            ...
            const t = e.event, r = "string" == typeof e.request ? new Request(e.request) : e.request, a = "params" in e ? e.params : void 0, s = new k(this, {
                event: t,
                request: r,
                params: a
        ...
        async _getResponse(e, r, a) {
            ...
            try {
                if (n = await this._handle(r, e), !n || "error" === n.type) throw new s("no-response", {
                    url: r.url
                });
            } catch (s) {
                if (s instanceof Error) for (const t of e.iterateCallbacks("handlerDidError")) if (n = await t({
                    error: s,
                    event: a,
                    request: r
                }), n) break;
                if (!n) throw s;
                t.log(`While responding to '${f(r.url)}', an ${s instanceof Error ? s.toString() : ""} error occurred. Using a fallback response provided by a handlerDidError plugin.`);
            }
    ...
    const L = (e, t) => `Using ${e} to respond to '${f(t.url)}'`, P = e => {
    ...
    function A(e) {
        ...
        if (!r) throw new s("add-to-cache-list-unexpected-type", {
            entry: e
        });
        if (!t) {
        ...
        const a = new URL(r, location.href), n = new URL(r, location.href);
    ...
    async function H(e, t) {
        ...
        if (r !== self.location.origin) throw new s("cross-origin-copy-response", {
            origin: r
        });
        const a = e.clone(), n = {
    ...
    class F extends E {
        constructor(e = {}) {
            e.cacheName = $(e.cacheName), super(e), this._fallbackToNetwork = !1 !== e.fallbackToNetwork, 
        ...
        async _handleFetch(e, r) {
            let a;
            const n = r.params || {};
            if (!this._fallbackToNetwork) throw new s("missing-precache-entry", {
                cacheName: this.cacheName,
                url: e.url
            });
            {
                t.warn(`The precached response for ${f(e.url)} in ${this.cacheName} was not found. Falling back to the network.`);
                const s = n.integrity, o = e.integrity, i = !o || o === s;
                if (a = await r.fetch(new Request(e, {
                    integrity: "no-cors" !== e.mode ? o || s : void 0
                })), s && i && "no-cors" !== e.mode) {
                    this._useDefaultCacheabilityPluginIfNeeded();
                    await r.cachePut(e, a.clone()) && t.log(`A response for ${f(e.url)} was used to "repair" the precache.`);
                }
            }
            {
                const s = n.cacheKey || await r.getCacheKey(e, "read");
                t.groupCollapsed("Precaching is responding to: " + f(e.url)), t.log(`Serving the precached url: ${f(s instanceof Request ? s.url : s)}`), 
                t.groupCollapsed("View request details here."), t.log(e), t.groupEnd(), t.groupCollapsed("View response details here."), 
                t.log(a), t.groupEnd(), t.groupEnd();
            }
            return a;
        }
        async _handleInstall(e, t) {
            ...
            if (!(await t.cachePut(e, r.clone()))) throw new s("bad-precaching-response", {
                url: e.url,
                status: r.status
            });
            return r;
        ...
        _useDefaultCacheabilityPluginIfNeeded() {
            let e = null, t = 0;
            for (const [r, a] of this.plugins.entries()) a !== F.copyRedirectedCacheableResponsesPlugin && (a === F.defaultPrecacheCacheabilityPlugin && (e = r), 
            a.cacheWillUpdate && t++);
    ...
    class j {
        ...
        addToCacheList(e) {
            ...
            for (const a of e) {
                ...
                const {cacheKey: e, url: n} = A(a), o = "string" != typeof a && a.revision ? "reload" : "default";
                if (this._urlsToCacheKeys.has(n) && this._urlsToCacheKeys.get(n) !== e) throw new s("add-to-cache-list-conflicting-entries", {
                    firstEntry: this._urlsToCacheKeys.get(n),
                    secondEntry: e
                });
                if ("string" != typeof a && a.integrity) {
                    if (this._cacheKeysToIntegrities.has(e) && this._cacheKeysToIntegrities.get(e) !== a.integrity) throw new s("add-to-cache-list-conflicting-integrities", {
                        url: n
                    });
            ...
            return K(e, (async () => {
                ...
                return function(e, r) {
                    ...
                    if (a || s) {
                        ...
                        D("View newly precached URLs.", e), D("View previously precached URLs.", r), t.groupEnd();
                    }
                }(a, s), {
                    updatedURLs: a,
                    notUpdatedURLs: s
        ...
        createHandlerBoundToURL(e) {
            ...
            if (!t) throw new s("non-precached-url", {
                url: e
            });
    ...
    class B extends d {
        constructor(e, r) {
            super((({request: a}) => {
                ...
                t.debug("Precaching did not find a match for " + f(a.url));
            }), e.strategy);
        }
    }
    e.StaleWhileRevalidate = class extends E {
        constructor(e = {}) {
        ...
        async _handle(e, r) {
            ...
            if (c) a.push(`Found a cached response in the '${this.cacheName}' cache. Will update with the network response in the background.`); else {
                a.push(`No response found in the '${this.cacheName}' cache. Will wait for the network response.`);
                try {
                    c = await n;
                } catch (e) {
            ...
            for (const e of a) t.log(e);
            if (P(c), t.groupEnd(), !c) throw new s("no-response", {
                url: e.url,
                error: o
            });
            return c;
    ...
    }, e.registerRoute = y, e.setCacheNameDetails = function(e) {
        ...
        })), "precache" in e && 0 === e.precache.length) throw new s("invalid-cache-name", {
            cacheNameId: "precache",
            value: e.precache
        });
        if ("runtime" in e && 0 === e.runtime.length) throw new s("invalid-cache-name", {
            cacheNameId: "runtime",
            value: e.runtime
        });
        if ("googleAnalytics" in e && 0 === e.googleAnalytics.length) throw new s("invalid-cache-name", {
